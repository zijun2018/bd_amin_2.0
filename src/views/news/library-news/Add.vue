...
<script>
  ...
  import {
    getAdminNewsDetail,
    postAdminNewsUploadPic,
    postAdminNewsAdd,
    postAdminNewsUpdate
  } from '../../../axios/api/admin-library-news';
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            if (this.$route.params.key) {
              ...
              return postAdminNewsUpdate({
                params: this.formData,
                success: () => {
                  message.success('提交成功');
                  this.$router.push({ name: 'library_news_list' }); // 跳转到列表页
                },
                error: () => {
                  message.error('提交失败');
                }
